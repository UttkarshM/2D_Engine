...
bool platform_create_window(int width, int height, char* title)
{
  ...
  // WS_CAPTION | WS_SYENENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_MAXIMIZEBOX
  ...
  // Fake Window initializing OpenGL
  {
    ...
    if(window == NULL)
    {
      EN_ASSERT(false, "Failed to create Windows Window");
      return false;
    }
    ...
    if(!fakeDC)
    {
      EN_ASSERT(false, "Failed to get HDC");
      return false;
    }
    ...
    if(!fakeRC)
    {
      EN_ASSERT(false, "Failed to create Render context");
      return false;
    }
  ...
  // Actual OpenGL initialization
  {
    ...
    if(!dc)
    {
      EN_ASSERT(false, "Failed to get DC");
      return false;
    }
    ...
    if(!wglChoosePixelFormatARB(dc, pixelAttribs,
                                ...
                                &numPixelFormats))
    {
      EN_ASSERT(0, "Failed to wglChoosePixelFormatARB");
      return false;
    }
    ...
    {
      EN_ASSERT(0, "Failed to SetPixelFormat");
      return true;
    }
    ...
    if(!rc)
    {
      EN_ASSERT(0, "Failed to crate Render Context for OpenGL");
      return false;
    }

    if(!wglMakeCurrent(dc, rc))
    {
      EN_ASSERT(0, "Faield to wglMakeCurrent");
      return false;
    }
  }
...
void* platform_load_dynamic_library(char* dll)
{
  ...
  EN_ASSERT(result, "Failed to load dll: %s", dll);
...
void* platform_load_dynamic_function(void* dll, char* funName)
{
  ...
  EN_ASSERT(proc, "Failed to load function: %s from DLL", funName);
...
bool platform_free_dynamic_library(void* dll)
{
  ...
  EN_ASSERT(freeResult, "Failed to FreeLibrary");
...
//audio , can replace these with any audio system u can find on the internet
...
void platform_update_audio(float dt)
{
  for(int soundIdx = 0; soundIdx < soundState->playingSounds.count; soundIdx++)
  {
    ...
    EN_ASSERT(sound.size > 0, "Sound has no Samples Size: %d", sound.size);
    EN_ASSERT(sound.data, "Sound has no Data!");
  ...
  for(int voiceIdx = 0; voiceIdx < MAX_CONCURRENT_SOUNDS; voiceIdx++)
  {
    ...
    if(voice->options & SOUND_OPTION_FADE_IN)
    {
      ...
      //SOUND_OPTION_FADE_IN will start
