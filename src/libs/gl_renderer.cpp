...
const char* TEXTURE_PATH =
  "/home/uttkarsh/game_engine/test/2D_Engine/assets/texture/TEXTURE_ATLAS (2).png";
...
GLuint gl_create_shader(int shaderType,
                        char* shaderPath,
...
bool gl_init(BumpAllocator* transientStorage)
{
  ...
  GLuint vertShaderID = gl_create_shader(
    GL_VERTEX_SHADER,
    "/home/uttkarsh/game_engine/test/2D_Engine/assets/shaders/quad.vert",
    transientStorage);

  GLuint fragShaderID = gl_create_shader(
    GL_FRAGMENT_SHADER,
    "/home/uttkarsh/game_engine/test/2D_Engine/assets/shaders/quad.frag",
    transientStorage);
  ...
  long long timestampVert = get_timestamp(
    "/home/uttkarsh/game_engine/test/2D_Engine/assets/shaders/quad.vert");
  long long timestampFrag = get_timestamp(
    "/home/uttkarsh/game_engine/test/2D_Engine/assets/shaders/quad.frag");
...

  {
    ...
    glBufferData(
      ...
      sizeof(Transform) * renderData->transforms.maxElements,
      renderData->transforms.elements,
      GL_DYNAMIC_DRAW);  //we use dynamic as we will still keep the same allocation when we change the data
  }

  // Uniforms
  {
    glContext.screenSizeID =
      glGetUniformLocation(glContext.programID, "screen_size");
  ...
  glEnable(
    GL_FRAMEBUFFER_SRGB);  // human eye doesnt interpret color in linear space , soo we need to convert the color in srgb back to human viewable form.
  glDisable(0x809D);
...
{
  {
    ...
    if (currentTimestamp > glContext.textureTimestamp) {
      ...
      char* data =
        (char*)stbi_load(TEXTURE_PATH, &width, &height, &nChannels, 4);
      if (data) {
  ...
  glUniformMatrix4fv(
    glContext.orthoProjectionID, 1, GL_FALSE, &orthoProjection.ax);

  // Copies transforms to the GPU
  glBufferSubData(GL_SHADER_STORAGE_BUFFER,
                  0,
                  sizeof(Transform) * renderData->transforms.count,
                  renderData->transforms.elements);
  glDrawArraysInstanced(
    ...
    renderData->transforms
      .count);  // The transform count is the glIndex that we use in the shader language
