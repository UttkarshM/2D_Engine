...
{
  return { gameState->player.pos.x - 4, gameState->player.pos.y - 8, 8, 16 };
}
...
{
  if (((hitbox.pos.x <= pos.x && hitbox.pos.x + hitbox.size.x >= pos.x) ||
       (hitbox.pos.x <= pos.x - 4 &&
        hitbox.pos.x + hitbox.size.x >=
          pos.x - 4))  //dies as he reaches halfway of the block
      && ((hitbox.pos.y <= pos.y && hitbox.pos.y + hitbox.size.y >= pos.y) ||
          (hitbox.pos.y <= pos.y - 4 &&
           hitbox.pos.y + hitbox.size.y >= pos.y - 4))) {
    return true;
  }
  return false;
}
...
bool save_tile_set()
{
  ...
  file.open(tilePath, std::ios::out);
  if (!file) {
  ...
  file1.open(tilePath, std::ios::app);
...
IVec2 parse_coords(std::string str)
{
  ...
  for (int i = 0; i < str.length(); i++) {
    if (str[i] == '-') {
      ...
      res[ind] =
        res[ind] * 10 + (str[i] - '0');  // Accumulate the current number
    }
  }
  IVec2 r = { res[0], res[1] };
  return r;
  ...
  float dt = UPDATE_DELAY;
  {
    ...
    if (just_pressed(MOUSE_MIDDLE)) {
      if (loaded_tiles.empty()) {
        EN_ERROR("couldnt load the tiles");
      }
      else {
        ...
        for (int y = 0; y < 5; y++) {
          for (int x = 0; x < 4; x++) {
            gameState->tileCoords.add(
              { tilesPosition.x + x * 8, tilesPosition.y + y * 8 });
          }
        }
