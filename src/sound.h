...
void play_sound(char* soundName, SoundOptions options = 0)
{
  ...
  if (!(options & SOUND_OPTION_START) && !(options & SOUND_OPTION_FADE_IN) &&
  ...
  for (int soundIdx = 0; soundIdx < soundState->allocatedSounds.count;
       soundIdx++) {
  ...
  if (wavFile) {
    if (wavFile->header.dataChunkSize >
        SOUNDS_BUFFER_SIZE - soundState->bytesUsed) {
      EN_ASSERT(
        0,
        "Exausted Sounds Buffer\nCapacity:\t%d\nBytes Used:\t%d\nSound Path:\t%s\nSound Size:\t%d",
        SOUNDS_BUFFER_SIZE,
        soundState->bytesUsed,
        sound.file,
        wavFile->header.dataChunkSize);
      return;
    }
