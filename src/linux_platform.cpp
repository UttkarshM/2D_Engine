...
bool platform_create_window(int width, int height, char* title)
{
  ...
  int fbcCount = 0;
  GLXFBConfig* fbc =
    glXChooseFBConfig(display, DefaultScreen(display), pixelAttribs, &fbcCount);

  if (!fbc) {
    EN_ASSERT(0, "glXChooseFBConfig() failed");
    return false;
  }
  ...
  GLXContext rc =
    glXCreateContextAttribsARB(display, fbc[0], NULL, true, contextAttribs);
  ...
  while (XPending(display)) {
    ...
    switch (event.type) {
      ...
      case ButtonRelease: {
        ...
        KeyCodeID keyCode =
          KeyCodeLookupTable[BUTTONS_KEYCODE_OFFSET + event.xbutton.button];
        Key* key = &input->keys[keyCode];
...
void* platform_load_dynamic_library(const char* dll)
{
  ...
  char* errstr = dlerror();
  if (errstr != NULL) {
    EN_ASSERT(false, "A dynamic linking error occurred: (%s)\n", errstr);
  }
  EN_ASSERT(lib, "Failed to load lib: %s", dll);
...
void* platform_load_dynamic_function(void* dll, const char* funName)
{
  ...
  EN_ASSERT(proc, "Failed to load function: %s from lib", funName);
...
{
  EN_ASSERT(dll, "No lib supplied!");
  int freeResult = dlclose(dll);
  EN_ASSERT(!freeResult, "Failed to dlclose");
...
void platform_fill_keycode_lookup_table()
{
  ...
  KeyCodeLookupTable[XKeysymToKeycode(display, XK_bracketleft)] =
    KEY_LEFT_BRACKET;
  KeyCodeLookupTable[XKeysymToKeycode(display, XK_bracketright)] =
    KEY_RIGHT_BRACKET;
  ...
  KeyCodeLookupTable[XKeysymToKeycode(display, XK_backslash)] =
    KEY_BACKWARD_SLASH;
  ...
  KeyCodeLookupTable[XKeysymToKeycode(display, XK_Scroll_Lock)] =
    KEY_SCROLL_LOCK;
